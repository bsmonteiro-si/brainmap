...
|------|-----------|--------------|
| **File** | `CanvasFileNode` | Reads `graphStore.nodes.get(filePath)` for title/type/tags/summary. Shows summary below title (3-line clamp). Double-click opens note (via `openNote`/`openPlainFile`/`openTab` depending on file type). Supports `.canvas` and `.excalidraw` files (opens in dedicated editors). Shows "missing reference" badge for broken links. Border color defaults to note type color. **Image preview:** for image files (`.png`, `.jpg`, etc.), loads an inline thumbnail via `resolveImagePath` + `convertFileSrc`; collapsible via chevron toggle (`imageExpanded` state); scales proportionally with node width (`width: 100%; height: auto`); uses `loading="lazy"` for off-screen nodes; triggers React Flow re-measure on load via `setNodes`; shows "Failed to load preview" on error. |
