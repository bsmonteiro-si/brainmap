...
import { ChevronDown, Plus, FolderPlus, X, RefreshCw, Diamond, Settings } from "lucide-react";
...
// ── Standalone SegmentSwitcher ───────────────────────────────────────────────
// Reusable in all three header layout modes.

export function SegmentSwitcher({ compact }: { compact?: boolean } = {}) {
  ...
  return (
    <div className={`segment-switcher${compact ? " segment-switcher--compact" : ""}`} ref={dropdownRef}>
      <button
      ...
      {dropdownOpen && (
        <div className="segment-switcher-dropdown">
          ...
          <button className="segment-switcher-add" onClick={handleAddSegment}>
            <Plus size={14} />
            <span>Open Folder as Segment</span>
          </button>
          <button className="segment-switcher-add" onClick={() => setShowCreateForm(!showCreateForm)}>
            <FolderPlus size={14} />
          ...
          {showCreateForm && (
            <div className="segment-create-form">
              ...
              <div className="segment-create-row">
                ...
                />
                <button className="segment-create-browse" type="button" onClick={handleCreateBrowse}>
                  Browse…
                </button>
              </div>
              <button
      ...
      >
        <RefreshCw size={compact ? 10 : 12} className={isLoading ? "spin" : ""} />
      </button>
    </div>
  );
}

// ── StatusBar (elevated mode only) ───────────────────────────────────────────
...
export function StatusBar() {
  ...
  const segmentName = activeSegment?.name ?? info.name;
  const monogram = segmentName.charAt(0).toUpperCase();

  return (
    <div className="status-bar status-bar--elevated">
      <div className="status-bar-left">
        <span className="status-bar-monogram">{monogram}</span>
        <SegmentSwitcher />
        <span className="status-bar-note-count">{info.node_count} notes · {info.edge_count} links</span>
      </div>
      <div className="status-bar-right">
        {activeNote && (
          <>
            ...
            <span className="status-bar-word-count">{wordCount} {wordCount === 1 ? "word" : "words"}</span>
          </>
        )}
        <button
          className="status-bar-gear"
